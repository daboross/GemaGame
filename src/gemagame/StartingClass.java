...
public class StartingClass extends Applet implements Runnable, KeyListener {
	...
	public void init() {
		...
		System.out.println("Set Background COllor");
		setFocusable(true);
		...
		System.out.println("Got document base");
	...
	public void start() {
		...
		for (int i = -100; i < 100; i++) {
			platformHandler.addPlatForm(i * 150,
					r.nextDouble() * this.getHeight(), 100,
					25 + r.nextDouble() * 5);
		}
	...
	public void run() {
		while (true) {
			...
			character.update(wPressed, sPressed, aPressed, dPressed, thisWidth,
					thisHeight);
			bg1.update();
			bg2.update();
			enemy10.update();
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i <= platformHandler.listLength(); i++) {
			g.drawImage(platform,
					(int) (platformHandler.xPosList()[i] + bg1.getDifX()),
